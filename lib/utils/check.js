...
function checkCliVersion(url, nowVersion) {
  return new Promise((resolve, reject) => {
    spinner.start('Checking @arrow-zb/cli version');
    request(url, function(err, res, body) {
      if(!err && res.statusCode === 200) {
        ...
        if(semver.lte(version, nowVersion)) {
          ...
          spinner.stop();
          warning(`You are creating a new project by @arrow-zb/cli, but the latest version is v${version}. \nPlease urgade your @arrow-zb/cli version.\n\n>> npm update @arrow-zb/cli -g`);
          process.exit(1);
        }
      }else {
        spinner.clear();
        warning('Failed to obtain the @arrow-zb/cli version from NPM. Please check and try again!');
        reject(err);
        process.exit(1);
      }
    })
